 ...
 **/
public class Ndef15693SchemeUtil {

    public static void main(String[] args) {
        new Ndef15693SchemeUtil().urlToHex(28,"weixin://dl/business/?t=wHOY6wCjIwt");
    }

    private int all_payload_length=0;//payload总长度
    private String all_payload_length_hex="";
    private String TNF_INFO="91";//标识符信息
    private String RTD_INFO1="55"; //代表‘U’ 声明ndef是url
    private String RTD_INFO2="54"; //代表‘T’ 声明ndef是文本
    ...
    public String urlToHex(int maxMemeryNum,String url){
        ...
        all_payload_length=5+url_info.length()+3+15+14;
        all_payload_length_hex=String.format("%02x", all_payload_length);
        url_info_hex=strTo16(url_info).toUpperCase();

        String type_pkg_hex = strTo16("android.com:pkg").toUpperCase();
        String type_pkg_length_hex = String.format("%02x", type_pkg_hex.length()/2);
        String name_pkg_hex = strTo16("com.tencent.mm").toUpperCase();
        String name_pkg_length_hex = String.format("%02x", name_pkg_hex.length()/2);

        memInfo="E140"+chip_info+"03"+all_payload_length_hex+TNF_INFO+"01"+url_info_length_hex
                +RTD_INFO1+RTD_TYPE+url_info_hex+ RTD_INFO2 + type_pkg_length_hex+ name_pkg_length_hex + type_pkg_hex+ name_pkg_hex;
